...
class DataInsertTestCase(TestCase):
    ...
    def test_short_cat(self):
        """
        test that categories_short.json file used to mock api call contains only the categories used
        in products json files.
        :return:
        """
        from_product_list_cat_ids = []

        fake_data_products = get_products_data_with_mock(self.fake_data_path, 'products', nb_files=3)
        ...
        from_product_list_cat_ids = list(set(sorted(from_product_list_cat_ids)))

        fake_data_categories = get_categories_data_with_mock(self.fake_data_path, 'categories_short')

        from_categories_list_cat_ids = list(set(sorted([cat['id'] for cat in fake_data_categories])))

        self.assertEqual(len(from_product_list_cat_ids), len(from_categories_list_cat_ids))
        self.assertEqual(from_product_list_cat_ids, from_categories_list_cat_ids)
