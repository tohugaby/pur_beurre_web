...
chmod +x update_app.sh && cat update_app.sh | ssh -o StrictHostKeyChecking=no -o BatchMode=yes -o SendEnv=LC_COMMIT_HASH -i deploy_key "$DEPLOY_USER@$DEPLOY_SERVER"